...
Filters AR and bank records by payment type and stores classified records in state.
"""
...
def _row_to_dict(row: pd.Series, col_map: dict, index: int) -> dict:
...
def classify_transactions(
    ...
    """
    Filter AR and bank records by payment type and store classified records in state.

    Args:
        ar_sheet           : AR sheet name
        bank_sheets        : list of bank sheet names
        ar_filter_column   : column to filter AR records on (e.g. 'Type' or canonical 'type')
        ar_filter_values   : accepted values (e.g. ['ACH', 'ECHECK'])
        bank_filter_column : column to filter bank records on (e.g. 'Category')
        bank_filter_values : accepted values (e.g. ['ACH'])
    """
    st       = state.get()
    ...
    if ar_df is None:
        return {"error": f"AR sheet '{ar_sheet}' not loaded."}
    ...
    if ar_filter_values and raw_filter_col in ar_df.columns:
        ...
        ar_filtered = ar_df[mask].copy().reset_index(drop=True)
    else:
        ar_filtered = ar_df.copy().reset_index(drop=True)

    ar_records = [_row_to_dict(row, ar_map, i) for i, row in ar_filtered.iterrows()]
    ...
    for sh in bank_sheets:
        ...
        if df is None:
            bank_counts[sh] = f"ERROR: not loaded"
            continue
    ...
    return {
        ...
        "next_step": "Call get_matching_rules to retrieve matching rules for the payment type.",
    }
